! function(e, s, i) {
    ...
    i(s).ready(function() {
        ...
        function get_messages(user_id, requestUser){
            ...
            fetch(url, {
                ...
                .then((data)=> {
                    ...
                    data[1].map(message => {
                        ...
                        let html = `
                        <div class="message media ${message.message_from == requestUser ? '' : 'reply'}">
                        ...
                        `
                        messages_div.innerHTML += html
                        el.find('.user-name').html(data[0]['name']);
        ...
        newChatForm.addEventListener('submit', e => {
            ...
            fetch('/send_message/', {
                ...
                body: JSON.stringify({'message': message, 'message_to': id_to, 'message_from': id_from})
            })
                .then((response)=> {                    
                    return response.json()
                })
                .then((data)=> {
                    ...
                    data[1].map(message => {
                        ...
                        let html = `
                        <div class="message media ${message.message_from == id_from ? '' : 'reply'}">
                            <figure class="user--online">
                                <a href="#">
                                    <img src="../img/users/3.jpg" class="rounded-circle" alt="">
                                </a>
        ...
        chatList.each(function(index) {
            ...
            $(this).find('.list-group a').on('click', function() {
                ...
                var el = $('.chat-panel');
                let a = document.querySelector('.chat-panel')
                console.log(a)
                if(!el.length) return;
                let user_id = this.dataset.id
                let requestUser = this.dataset.action

                get_messages(user_id, requestUser)
